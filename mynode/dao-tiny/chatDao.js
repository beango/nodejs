...
exports.add = function(chatPost,callback) {
	tiny(dbpath + 'Northwind.Chat.tiny', function(err, db) {
      db.find()
      .desc('chatID')
      .limit(1)(function(err, top1) {
        chatPost.chatID = 1;
        if(top1!=null&&top1.length>0){
            chatPost.chatID = parseInt(top1[0].chatID)+1;
        }
        db.set('Chat'+chatPost.chatID, chatPost, function(err) {
            if (err)
              callback(err);
            callback(null);
        });
